...
function dashboardFor(role: string, data: any): DashboardView {
  ...
  if (role === 'cost controller') {
    ...
    return base({ subtitle: 'Suppliers, articles, units and quotation data.', tasks: [task('Suppliers', activeSuppliers, 'Approved supplier records', 'local_shipping', 'suppliers', 'Suppliers'), task('Supplier quotations', activePrices, 'Current item prices', 'request_quote', 'supplierItems', 'Supplier quotations'), task('Articles', items, 'Article master records', 'inventory_2', 'items', 'Articles / items'), task('UOM conversions', itemUnits, `${missingConversions.length} article(s) without conversion`, 'calculate', 'itemUnits', 'UOM conversions', missingConversions.length ? 'warning' : 'good')], activities: activity(supplierItems, r=>`${r.article || 'Article'} · ${r.supplier || 'Supplier'}`, r=>`${money(r.price || 0)} per ${r.unit || 'unit'} · Quote ${r.quotationReference || 'not recorded'}`, 'supplierItems', 'Supplier quotations'), queueTitle: 'Supplier quotation catalogue', queueHint: 'Supplier + article + quotation + price', queueRoute: 'supplierItems', queueRouteLabel: 'Supplier quotations', context: [{label:'Units configured',value:String(uoms.length)},{label:'Active suppliers',value:String(activeSuppliers.length)}], primaryAction: { title: 'Supplier & quotation setup', hint: 'Manage suppliers and quotation records.', label: 'Open suppliers', route: 'suppliers', icon: 'local_shipping' } }) }
