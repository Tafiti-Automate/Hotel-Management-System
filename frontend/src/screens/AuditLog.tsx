...
export default function AuditLog() {
  ...
  return <div style={{ maxWidth: 1400, margin: '0 auto' }}>
    ...
    <div style={{ ...panel, overflow: 'hidden', borderRadius: '0 0 8px 8px' }}>
      ...
      {!loading && visible.map((row) => <div key={String(row.id)} style={auditRow}><span style={{ fontWeight: 700, color: 'var(--text)' }}>{String(row.action).replace(/_/g, ' ')}</span><span>{String(row.entity_type)}</span><span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11 }}>{String(row.entity_id || '—').slice(0, 12)}</span><span>{new Date(String(row.created_at)).toLocaleString()}</span></div>)}
...
const auditRow: CSSProperties = { display: 'grid', gridTemplateColumns: '1.2fr 1.5fr 1fr 1.2fr', gap: 12, padding: '12px 16px', borderBottom: '1px solid var(--border)', color: 'var(--text-muted)', fontSize: 12 }
