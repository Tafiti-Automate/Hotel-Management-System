...
export default function SupplierQuotationManagement() {
  ...
  return <div style={{ maxWidth: 1420, margin: '0 auto' }}>
    ...
    <section style={{ ...card, overflow:'hidden' }}>
      ...
      <div className="supplier-quote-table-head" style={{ display:'grid', gridTemplateColumns:'minmax(170px,1.25fr) minmax(170px,1.25fr) 100px 125px minmax(150px,1fr) 90px 90px 32px', gap:10, padding:'10px 14px', background:'var(--surface-2)', borderBottom:'1px solid var(--border)', color:'var(--text-faint)', fontSize:10.25, fontWeight:750, textTransform:'uppercase' }}>
...
function Status({ value }: { value:string }) {
  ...
  return <span style={{ justifySelf:'start', padding:'4px 8px', borderRadius:999, color:active?'var(--good)':'var(--text-muted)', background:active?'var(--good-soft)':'var(--surface-2)', fontSize:9.5, fontWeight:750 }}>{value || 'Inactive'}</span>
}

const mainText: CSSProperties = { display:'block', color:'var(--text)', fontSize:11.5, fontWeight:650, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }
const mutedText: CSSProperties = { color:'var(--text-muted)', fontSize:11.25 }
const subText: CSSProperties = { display:'block', marginTop:2, color:'var(--text-faint)', fontSize:9.75, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }
