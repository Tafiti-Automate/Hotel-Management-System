  ...
  return <div style={{ maxWidth: 1420, margin: '0 auto' }}>
    ...
    <div className="supplier-master-grid" style={{ display: 'grid', gridTemplateColumns: selected ? 'minmax(500px,1fr) minmax(360px,.8fr)' : '1fr', gap: 14, alignItems: 'start' }}>
      <section style={{ ...card, overflow: 'hidden' }}>
        ...
        <div className="supplier-table-head" style={{ display: 'grid', gridTemplateColumns: 'minmax(180px,1.3fr) 1fr 1fr 90px 30px', gap: 10, padding: '10px 15px', color: 'var(--text-faint)', fontSize: 12, fontWeight: 750, textTransform: 'uppercase', borderBottom: '1px solid var(--border)', background: 'var(--surface-2)' }}><span>Supplier</span><span>TIN / Contact</span><span>Items supplied</span><span>Status</span><span /></div>
        {filtered.map((supplier) => {
          ...
          return <button key={text(supplier.id)} type="button" onClick={() => setSelectedId(text(supplier.id))} className="hover-surface2" style={{ width:'100%', display:'grid', gridTemplateColumns:'minmax(180px,1.3fr) 1fr 1fr 90px 30px', gap:10, alignItems:'center', padding:'12px 15px', border:0, borderBottom:'1px solid var(--border)', background:active?'var(--accent-soft)':'transparent', textAlign:'left', cursor:'pointer', font:'inherit' }}>
            <span style={{ minWidth: 0 }}><strong style={{ display:'block', color:'var(--text)', fontSize:12.5, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{text(supplier.name)}</strong><span style={{ display:'block', marginTop:3, color:'var(--text-faint)', fontSize: 12, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{text(supplier.email) || text(supplier.phone) || 'No contact recorded'}</span></span>
            <span style={{ color:'var(--text-muted)', fontSize: 12 }}><span style={{ display:'block' }}>{text(supplier.tinNumber) || 'TIN not recorded'}</span><span style={{ display:'block', marginTop:2, color:'var(--text-faint)' }}>{text(supplier.contact) || 'No contact person'}</span></span>
            <span style={{ color:'var(--text)', fontSize: 12, fontWeight:650 }}>{quoteCount} article{quoteCount===1?'':'s'}</span>
      ...
      {selected && <aside style={{ ...card, overflow: 'hidden', position: 'sticky', top: 0 }}>
        <div style={{ padding: 18, borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}><div><div style={{ color:'var(--text-faint)', fontSize: 12, fontWeight:750, textTransform:'uppercase' }}>Supplier profile</div><h2 style={{ margin:'4px 0 4px', color:'var(--text)', fontSize:20 }}>{text(selected.name)}</h2><Status value={text(selected.status)} /></div><button type="button" onClick={() => app.openEdit(text(selected.id))} style={secondary}><Icon name="edit" size={16} />Edit</button></div>
        ...
        <div style={{ padding: 18 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:10, marginBottom:10 }}><div><div style={{ color:'var(--text)', fontSize:13.5, fontWeight:750 }}>Items & quotations</div><div style={{ marginTop:2, color:'var(--text-faint)', fontSize: 12 }}>Only quotation-backed supplier/article prices</div></div><button type="button" onClick={() => app.navTo('supplierItems','Supplier quotations')} style={link}>Open catalogue <Icon name="arrow_forward" size={14} /></button></div>
          <div style={{ border:'1px solid var(--border)', borderRadius:8, overflow:'hidden' }}>
            <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.25fr) .7fr .8fr', gap:8, padding:'8px 10px', color:'var(--text-faint)', background:'var(--surface-2)', fontSize: 11.5, fontWeight:750, textTransform:'uppercase' }}><span>Article</span><span>UOM</span><span>Current quote</span></div>
            {prices.slice(0,8).map((row) => <div key={text(row.id)} style={{ display:'grid', gridTemplateColumns:'minmax(0,1.25fr) .7fr .8fr', gap:8, padding:'10px', borderTop:'1px solid var(--border)', fontSize: 12 }}><span><strong style={{ color:'var(--text)' }}>{text(row.article)}</strong><span style={{ display:'block', marginTop:2, color:'var(--text-faint)', fontSize: 11.5 }}>{text(row.quotationReference) ? `Quote ${text(row.quotationReference)}` : 'Quotation reference not recorded'}</span></span><span style={{ color:'var(--text-muted)' }}>{text(row.unit) || '—'}</span><span style={{ color:'var(--text)', fontWeight:700 }}>{money(row.price || 0)}</span></div>)}
            {!prices.length && <div style={{ padding:18, color:'var(--text-faint)', fontSize: 12 }}>No supplier-item quotation has been registered yet.</div>}
          </div>
        </div>
      </aside>}
    </div>
...
function Info({ label, value }: { label:string; value:unknown }) { return <div style={{ padding:'9px 10px', border:'1px solid var(--border)', borderRadius:7, background:'var(--surface-2)', minWidth:0 }}><div style={{ color:'var(--text-faint)', fontSize: 11.5, fontWeight:750, textTransform:'uppercase' }}>{label}</div><div style={{ marginTop:3, color:'var(--text)', fontSize: 12, fontWeight:650, overflow:'hidden', textOverflow:'ellipsis' }}>{text(value) || '—'}</div></div> }
function Status({ value }: { value:string }) { const active=value.toLowerCase()==='active'; return <span style={{ display:'inline-flex', padding:'4px 8px', borderRadius:999, color:active?'var(--good)':'var(--text-muted)', background:active?'var(--good-soft)':'var(--surface-2)', fontSize: 11.5, fontWeight:750 }}>{value || 'Inactive'}</span> }
...
const secondary: CSSProperties = { minHeight:34, display:'inline-flex', alignItems:'center', gap:6, padding:'0 10px', border:'1px solid var(--border)', borderRadius:7, background:'var(--surface)', color:'var(--text-muted)', font:'inherit', fontSize: 12, fontWeight:650, cursor:'pointer' }
const link: CSSProperties = { border:0, background:'transparent', color:'var(--accent)', display:'inline-flex', alignItems:'center', gap:4, font:'inherit', fontSize: 12, fontWeight:700, cursor:'pointer' }
