...
import { createBackendRecord, downloadControlledPurchaseOrder, downloadProcurementAttachment, errorMessage, fetchHotels, readBackendPayload, readBackendRecords, runBackendAction, uploadProcurementAttachment, type HotelRecord } from '../lib/api'
...
export default function ProcurementWorkbench() {
  ...
  const requisitionLabel = (row: Row) => `${id(row.source_store_requisition_no) || id(row.requisition_number) || `PR-${id(row.id).slice(0, 8).toUpperCase()}`} · ${id(row.procurement_source) === 'store_purchase' ? 'Store Purchase Request' : isProcurementHandoffSource(row.procurement_source) ? 'Store Requisition' : 'Manual'} · ${id(row.reason)}`
  ...
  const roleApprovalQueueOrders = useMemo(() => {
    ...
    if (role === 'procurement officer') return []
  ...
  const procurementQueues = useMemo(() => {
    const storeRequisitions = scopedData.requisitions.filter((row) =>
      isProcurementHandoffSource(row.procurement_source) &&
    ...
    <div className="enterprise-workspace procurement-workbench" style={{ maxWidth: 1480, margin: '0 auto' }}>
      ...
      {isProcurementRole ? <div className="procurement-queue-tabs" style={{ ...card, padding: 8, marginBottom: 16, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 6 }}>
        {([
          ...
          ...(role === 'procurement manager' ? [['purchasing', 'Level 1 Approval', procurementQueues.purchasing] as ['allocation' | LpoQueue, string, number]] : []),
...
function RequestPanel({ data, form, setForm, requisitionLabel, items }: any) {
  const incoming = data.requisitions.filter((row: Row) => isProcurementHandoffSource(row.procurement_source))
  ...
  return <Panel title="Incoming purchase requests" note="Select a purchase request to review its articles, quantities and destination.">
    <Field label="Purchase request"><Select value={form.requisition} onChange={(v) => setForm({ requisition: v })} rows={incoming} label={requisitionLabel} /></Field>
    {!incoming.length && <Hint>No purchase requests are waiting for Procurement.</Hint>}
...
function QuotePanel({ data, form, setForm, busy, run, names, suppliers, supplierItems, items, itemUnits, onContinueToLpo }: any) {
  const requisitions = data.requisitions.filter((row: Row) => isProcurementHandoffSource(row.procurement_source) && ['approved','partially_ordered'].includes(id(row.status)))
  ...
  const confirmedPrice = quotedPrice
  ...
  const activeReqLines = reqLines.filter((line: Row) => num(line.approved_quantity ?? line.quantity) > 0 && !id(line.rejection_stage))
  const allocatedCount = activeReqLines.filter((line: Row) => line.procurement_supplier_price && num(line.procurement_quantity) > 0 && num(line.procurement_unit_cost) > 0).length
  const allAllocated = Boolean(activeReqLines.length) && allocatedCount === activeReqLines.length
  ...
  const selectArticle = (value: string) => {
    ...
    setForm({
      ...
      sourceQuantity: line?.approved_quantity ?? line?.quantity ?? '',
      sourceReason: line?.rejection_reason || line?.procurement_note || '',
  ...
  return <Panel title={`Store Requisition · ${id(selectedRequisition.source_store_requisition_no) || id(selectedRequisition.requisition_number)}`} note="">
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 12 }}>
      ...
      <span style={{ padding: '5px 8px', borderRadius: 999, background: allAllocated ? 'var(--good-soft)' : 'var(--warn-soft)', color: allAllocated ? 'var(--good)' : 'var(--warn)', fontSize: 10.5, fontWeight: 750 }}>{allocatedCount}/{activeReqLines.length} active items allocated</span>
    </div>
    ...
    {selectedCatalogue && <>
      ...
      <Action tone="good" disabled={busy || !form.reqLine || !form.cataloguePrice || enteredOrderQuantity <= 0 || allocationExceedsRequest || confirmedPrice <= 0} onClick={() => run(
...
function LpoPanel({ data, form, setForm, busy, run, names, suppliers, items, itemUnits, canManage, role, userName, lpoQueue, setLpoQueue }: any) {
  ...
  const readyRequisitions = data.requisitions.filter((row: Row) => {
    ...
    const activeLines = reqLines.filter((line: Row) => num(line.approved_quantity ?? line.quantity) > 0 && !id(line.rejection_stage))
    return Boolean(activeLines.length) && activeLines.every((line: Row) => Boolean(line.procurement_supplier_price) && num(line.procurement_quantity) > 0 && num(line.procurement_unit_cost) > 0)
  })
  ...
  const canDecidePurchasing = ['procurement manager', 'procurement officer', 'system administrator'].includes(role)
  ...
  type ApprovalQuantityStage = 'purchasing' | 'finance' | 'management'
  const upstreamQuantity = (orderLine: Row, approvalStage: ApprovalQuantityStage) => {
    if (approvalStage === 'purchasing') return num(orderLine.procurement_quantity ?? orderLine.quantity)
    if (approvalStage === 'finance') return num(orderLine.purchasing_approved_quantity ?? orderLine.procurement_quantity ?? orderLine.quantity)
    return num(orderLine.finance_approved_quantity ?? orderLine.purchasing_approved_quantity ?? orderLine.procurement_quantity ?? orderLine.quantity)
  }
  const savedStageQuantity = (orderLine: Row, approvalStage: ApprovalQuantityStage) => approvalStage === 'purchasing'
    ? orderLine.purchasing_approved_quantity
    : approvalStage === 'finance'
      ? orderLine.finance_approved_quantity
      : orderLine.management_approved_quantity
  const savedStageReason = (orderLine: Row, approvalStage: ApprovalQuantityStage) => approvalStage === 'purchasing'
    ? orderLine.purchasing_reduction_reason
    : approvalStage === 'finance'
      ? orderLine.finance_reduction_reason
      : orderLine.management_reduction_reason
  const quantityFormKey = (approvalStage: ApprovalQuantityStage, orderLine: Row) => `approvalQty:${approvalStage}:${id(orderLine.id)}`
  const reasonFormKey = (approvalStage: ApprovalQuantityStage, orderLine: Row) => `approvalReason:${approvalStage}:${id(orderLine.id)}`
  const decisionQuantity = (orderLine: Row, approvalStage: ApprovalQuantityStage) => form[quantityFormKey(approvalStage, orderLine)] ?? savedStageQuantity(orderLine, approvalStage) ?? upstreamQuantity(orderLine, approvalStage)
  const decisionReason = (orderLine: Row, approvalStage: ApprovalQuantityStage) => id(form[reasonFormKey(approvalStage, orderLine)] ?? savedStageReason(orderLine, approvalStage) ?? '')
  const setDecisionQuantity = (orderLine: Row, approvalStage: ApprovalQuantityStage, value: unknown) => setForm({ ...form, [quantityFormKey(approvalStage, orderLine)]: value })
  const setDecisionReason = (orderLine: Row, approvalStage: ApprovalQuantityStage, value: unknown) => setForm({ ...form, [reasonFormKey(approvalStage, orderLine)]: value })
  const approvalDecisions = (approvalStage: ApprovalQuantityStage) => lines.map((orderLine: Row) => ({
    id: id(orderLine.id),
    approved_quantity: num(decisionQuantity(orderLine, approvalStage)),
    reason: decisionReason(orderLine, approvalStage),
  }))
  const approvalDecisionsValid = (approvalStage: ApprovalQuantityStage) => Boolean(lines.length)
    && lines.every((orderLine: Row) => {
      const upstream = upstreamQuantity(orderLine, approvalStage)
      const quantity = num(decisionQuantity(orderLine, approvalStage))
      return quantity >= 0 && quantity <= upstream && (quantity >= upstream || Boolean(decisionReason(orderLine, approvalStage).trim()))
    })
    && lines.some((orderLine: Row) => num(decisionQuantity(orderLine, approvalStage)) > 0)
  const quantityReview = (approvalStage: ApprovalQuantityStage, stageLabel: string) => <section style={{ margin: '12px 0', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
    <div style={{ padding: '9px 11px', background: 'var(--surface-2)', borderBottom: '1px solid var(--border)', fontSize: 11.5, fontWeight: 800 }}>Quantity review · {stageLabel}</div>
    {lines.map((orderLine: Row) => {
      const upstream = upstreamQuantity(orderLine, approvalStage)
      const quantity = num(decisionQuantity(orderLine, approvalStage))
      const reason = decisionReason(orderLine, approvalStage)
      const rejectedEarlier = upstream <= 0
      return <div key={`${approvalStage}-${id(orderLine.id)}`} style={{ display: 'grid', gridTemplateColumns: 'minmax(150px,1.2fr) 95px 115px 120px minmax(150px,1fr) 105px', gap: 8, alignItems: 'center', padding: '10px 11px', borderBottom: '1px solid var(--border)', fontSize: 11 }}>
  ...
  const draftQuantityKey = (orderLine: Row) => `draftQty:${id(orderLine.id)}`
  const draftQuantity = (orderLine: Row) => num(form[draftQuantityKey(orderLine)] ?? orderLine.quantity)
  const setDraftQuantity = (orderLine: Row, value: unknown) => setForm({ ...form, [draftQuantityKey(orderLine)]: value })
  const maxDraftQuantity = (orderLine: Row) => {
    const article = items.find((item: Row) => id(item.id) === id(orderLine.item))
    const factor = conversionFactorFor(article, orderLine.unit, itemUnits)
    ...
    return Math.max(0, floorPurchaseQuantity(remainingBase / factor))
  }
  const draftQuantitiesValid = Boolean(lines.length) && lines.every((orderLine: Row) => {
    const quantity = draftQuantity(orderLine)
    const maximum = maxDraftQuantity(orderLine)
    return quantity > 0 && maximum > 0 && quantity <= maximum + 0.000001
  })
  const draftTotal = lines.reduce((sum: number, orderLine: Row) => sum + (draftQuantity(orderLine) * num(orderLine.unit_cost)), 0)
  const saveDraftQuantitiesAndSubmit = async () => {
    await runBackendAction('purchase-orders', id(order.id), 'update-draft-quantities', {
      lines: lines.map((orderLine: Row) => ({ id: id(orderLine.id), quantity: draftQuantity(orderLine) })),
    })
    return runBackendAction('purchase-orders', id(order.id), 'submit-for-approval')
  }
  ...
  return <Panel title={panelTitle} note="">
    ...
    {lpoQueue === 'prepare' && order && canManage && <>
      <LpoSummary
        ...
        quantityMax={maxDraftQuantity}
        onQuantityChange={setDraftQuantity}
        totalOverride={draftTotal}
      />
      <Action tone="good" disabled={busy || !draftQuantitiesValid} onClick={() => run(() => saveDraftQuantitiesAndSubmit(), 'LPO sent to Purchasing Manager')}>Send for Level 1 Approval</Action>
    </>}
    ...
    {lpoQueue === 'purchasing' && order && <>
      ...
      {canDecidePurchasing && <>
        {quantityReview('purchasing', 'Purchasing Manager')}
        <Action tone="good" disabled={busy || !currentApproval || !approvalDecisionsValid('purchasing')} onClick={() => run(() => reviewAndApprove('purchasing'), 'Level 1 approved and sent to Financial Manager')}>Approve LPO</Action>
        {!form.showReject ? <Action tone="danger" disabled={busy} onClick={() => setForm({ ...form, showReject: true })}>Reject entire LPO</Action> : <><Field label="Rejection reason *"><Input value={form.approvalComments || ''} onChange={(v) => setForm({ ...form, approvalComments: v })} /></Field><Action tone="danger" disabled={busy || !id(form.approvalComments).trim()} onClick={() => run(() => runBackendAction('purchase-orders', id(order.id), 'reject', { comments: form.approvalComments }), 'LPO rejected')}>Confirm full rejection</Action></>}
      </>}
    </>}
    ...
    {lpoQueue === 'finance' && order && <>
      ...
      {canDecideFinance && <>
        {quantityReview('finance', 'Financial Manager')}
        <Action tone="good" disabled={busy || !currentApproval || !approvalDecisionsValid('finance')} onClick={() => run(() => reviewAndApprove('finance'), 'LPO approved and sent to General Manager')}>Approve LPO</Action>
        {!form.showReject ? <Action tone="danger" disabled={busy} onClick={() => setForm({ ...form, showReject: true })}>Reject entire LPO</Action> : <><Field label="Rejection reason *"><Input value={form.approvalComments || ''} onChange={(v) => setForm({ ...form, approvalComments: v })} /></Field><Action tone="danger" disabled={busy || !id(form.approvalComments).trim()} onClick={() => run(() => runBackendAction('purchase-orders', id(order.id), 'reject', { comments: form.approvalComments }), 'LPO rejected')}>Confirm full rejection</Action></>}
      </>}
    </>}
    ...
    {lpoQueue === 'management' && order && <>
      ...
      {canDecideManagement && <>
        {quantityReview('management', 'General Manager')}
        <Action tone="good" disabled={busy || !currentApproval || !approvalDecisionsValid('management')} onClick={() => run(async () => { const result = await reviewAndApprove('management'); setLpoQueue('history'); return result }, 'Final LPO approval recorded', (result: unknown) => ({ order: id((result as Row).id) }))}>Approve LPO</Action>
        {!form.showReject ? <Action tone="danger" disabled={busy} onClick={() => setForm({ ...form, showReject: true })}>Reject entire LPO</Action> : <><Field label="Rejection reason *"><Input value={form.approvalComments || ''} onChange={(v) => setForm({ ...form, approvalComments: v })} /></Field><Action tone="danger" disabled={busy || !id(form.approvalComments).trim()} onClick={() => run(async () => { const result = await runBackendAction('purchase-orders', id(order.id), 'reject', { comments: form.approvalComments }); setLpoQueue('history'); return result }, 'LPO rejected', (result: unknown) => ({ order: id((result as Row).id) }))}>Confirm full rejection</Action></>}
      </>}
    </>}
...
function LpoSummary({ order, lines, names, quantityValue, quantityMax, onQuantityChange, totalOverride }: { order: Row; lines: Row[]; names: Record<string, Map<string, string>>; quantityValue?: (row: Row) => number; quantityMax?: (row: Row) => number; onQuantityChange?: (row: Row, value: unknown) => void; totalOverride?: number }) {
  ...
  return <section className="lpo-summary-card" style={{ margin: '10px 0 14px', overflow: 'hidden', border: '1px solid var(--border)', borderRadius: 8 }}>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 8, padding: 11, background: 'var(--surface-2)' }}>
      ...
      <ReadOnlyValue label="LPO total" value={money(totalOverride ?? order.total_amount)} />
    </div>
    ...
    {lines.map((row) => {
      const shownQuantity = quantityValue ? quantityValue(row) : num(row.approved_quantity ?? row.quantity)
      const maximum = quantityMax ? quantityMax(row) : undefined
      return <div key={id(row.id)} style={{ padding: '9px 11px', display: 'grid', gridTemplateColumns: 'minmax(0,1.4fr) .6fr .7fr .8fr', gap: 8, alignItems: 'center', borderTop: '1px solid var(--border)', color: 'var(--text-muted)', fontSize: 10.8 }}>
        <strong style={{ color: 'var(--text)' }}>{names.items.get(id(row.item)) || id(row.item)}</strong>
        {onQuantityChange ? <input type="number" min="0.01" max={maximum} step="0.01" value={shownQuantity} onChange={(event) => onQuantityChange(row, event.target.value)} aria-label={`Quantity for ${names.items.get(id(row.item)) || id(row.item)}`} style={{ ...control, height: 34, minWidth: 0 }} /> : <span>{id(row.approved_quantity ?? row.quantity)}</span>}
        <span>{money(row.unit_cost)}</span>
        <span>{money(shownQuantity * num(row.unit_cost))}</span>
      </div>
    })}
...
function StageTable({ stage, lpoQueue, data, names, role, onSelect }: { stage: Stage; lpoQueue: LpoQueue; data: Datasets; names: Record<string, Map<string, string>>; role: string; onSelect: (row: Row) => void }) {
  ...
  if (stage === 'quote') {
    const requisitions = data.requisitions.filter((row) => {
      if (!isProcurementHandoffSource(row.procurement_source) || !['approved','partially_ordered'].includes(id(row.status))) return false
      const lines = data.requisitionItems.filter((line) => id(line.requisition) === id(row.id))
      return lines.some((line) => !line.procurement_supplier_price || num(line.procurement_quantity) <= 0 || num(line.procurement_unit_cost) <= 0)
    })
    return <div>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}><strong style={{ fontSize: 12.8 }}>New Purchase Requests</strong><span style={{ color: 'var(--text-faint)', fontSize: 10.5 }}>{requisitions.length} pending</span></div>
      ...
      {!requisitions.length && <div style={{ padding: 42, textAlign: 'center', color: 'var(--text-faint)', fontSize: 12 }}>No new purchase requests are waiting for Procurement.</div>}
    </div>
  }
  ...
  if (stage === 'lpo') {
    if (lpoQueue === 'purchasing') { rows = role === 'procurement manager' ? (Array.isArray(data.approvalQueueOrders) ? data.approvalQueueOrders : data.orders.filter((row) => id(row.status) === 'pending_approval' && isPurchasingApproval(row))) : []; title = 'Level 1 · Purchasing Manager' }
