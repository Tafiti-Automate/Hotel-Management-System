...
function ReceiptPanel({ data, form, setForm, busy, run, orderLabel, receiptLabel, names, employees, stores }: any) {
  ...
  return <Panel title="Goods receipt note" note="Record supplier deliveries.">
    ...
    {editing && <><Action disabled={busy || Boolean(editing.inventory_changes_applied)} onClick={() => run(() => updateBackendRecord('grn-items', id(editing.id), { goods_receipt: form.receipt, purchase_order_item: form.orderLine, store: form.store, quantity_received: num(form.quantity), unit_cost: num(form.cost), expiry_date: form.expiry || null }), 'Delivered line corrected')}>Save GRN correction</Action><Action tone="danger" disabled={busy || Boolean(editing.inventory_changes_applied)} onClick={() => run(() => deleteBackendPath('grn-items', id(editing.id)), 'Delivered line removed')}>Remove delivered line</Action></>}
  </Panel>
}
        ...
        {stage === 'request' ? null : <section style={{ marginTop: 25 }}>
          ...
          <div style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
            {lines.map((line) => {
              ...
              return <div key={id(line.id)} style={{ display: 'flex', justifyContent: 'space-between', gap: 14, padding: '13px 14px', borderBottom: '1px solid var(--border)' }}><div><div style={{ color: 'var(--text)', fontSize: 12.5, fontWeight: 650 }}>{lineName(line)}</div><div style={{ marginTop: 4, color: 'var(--text-faint)', fontSize: 11.5 }}>{secondaryText}</div></div>{line.selected && <span style={{ color: 'var(--good)', fontSize: 11.5, fontWeight: 700 }}>Selected</span>}</div>
            })}
