...
export async function fetchBackendData(): Promise<BackendDataResult> {
  ...
  data.balances = raw.balances.map((row) => {
    ...
    return {
      ...
      calculatedReserved: num(row.calculated_reserved_quantity),
      reservationVariance: num(row.reservation_variance),
      reservedAllocations: Array.isArray(row.reserved_allocations) ? row.reserved_allocations.map((allocation: Row) => ({
        request: text(allocation.request_number),
        department: text(allocation.department),
        requester: text(allocation.requester),
        approved: num(allocation.approved_quantity),
        issued: num(allocation.issued_quantity),
        outstanding: num(allocation.outstanding_quantity),
        status: titleCaseStatus(allocation.status),
      })) : [],
