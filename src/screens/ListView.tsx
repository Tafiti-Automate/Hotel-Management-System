  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, boxShadow: 'var(--shadow)', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: grid, borderBottom: '1px solid var(--border)', padding: '0 8px' }}>
          {conf.cols.map((c) => <div key={c.key} style={headStyle(c)}>{c.label}</div>)}
          <div />
        </div>

        {data.map((r) => (
          <div key={r.id} onClick={() => conf.detail && onRowClick(r.id)} className="hover-surface2" style={{ display: 'grid', gridTemplateColumns: grid, alignItems: 'center', borderBottom: '1px solid var(--border)', padding: '0 8px', cursor: conf.detail ? 'pointer' : 'default' }}>
            {conf.cols.map((c) => {
            ...
            <div style={{ padding: '10px 8px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px', fontSize: 12, color: 'var(--text-muted)' }}>
